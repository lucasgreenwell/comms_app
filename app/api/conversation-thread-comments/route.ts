...
export async function GET(request: Request) {
  ...
  try {
    ...
    const { data: comments, error: commentsError } = await supabase
      ...
      .select('id, user_id, message_id, conversation_id, content, created_at')
...
export async function POST(request: Request) {
  try {
    ...
    const { messageId, conversationId, userId, content } = body

    if (!messageId || !conversationId || !userId || !content) {
